...
void ws_quadratic_regularizer_drot_torch_float32(
    const float *c,
    const float *p,
    const float *q,
    const int n_rows,
...
// const float v_a = (_k + float(1)) / _n;
// const float v_b = (_k + float(1)) / _m;
// const float v_alpha = // sum(a) / (_m + _n);
// const float v_beta = 0;
...
void warm_start_q_drot_cuda(
    ...
    TORCH_CHECK((int) x.size(0) == n_cols, "X.size(0) does not match with q.numel()");
    TORCH_CHECK((int) x.size(1) == n_rows, "X.size(1) does not match with p.numel()");

    //TORCH_CHECK((int) phi1.size(0) == n_cols, "phi1.size(0) does not match with p.numel()");
    //TORCH_CHECK((int) phi2.size(0) == n_rows, "phi2.size(0) does not match with q.numel()");

    //TORCH_CHECK((int) a.size(0) == n_cols, "a.size(0) does not match with p.numel()");
    //TORCH_CHECK((int) b.size(0) == n_rows, "b.size(0) does not match with q.numel()");
    ...
    float c_alpha = a.sum().item<float>(); // / (float(n_rows) + float(n_cols));
    auto aux = torch::empty({5}, options);
    float *aux_ptr = aux.data_ptr<float>();
    cudaMemset(aux_ptr, 0, 5*sizeof(float));
    cudaMemcpy(&aux_ptr[4], &c_alpha, sizeof(float), cudaMemcpyHostToDevice);

    // run
    ws_quadratic_regularizer_drot_torch_float32(
        c.data_ptr<float>(),
        p.data_ptr<float>(),
        q.data_ptr<float>(),
        n_rows,
